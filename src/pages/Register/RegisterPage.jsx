...
import { FaUser, FaEnvelope, FaLock, FaPhone, FaImage, FaGlobe, FaCalendarAlt } from 'react-icons/fa';
...
const RegisterPage = () => {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      const res = await fetch('https://your-api-url/users', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(userData),
      });

      if (!res.ok) throw new Error('User data save failed.');
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-[#F2EFE7] p-4">
      <div className="w-full max-w-lg p-8 rounded-xl bg-white shadow-lg border border-[#9ACBD0]">
        <h2 className="text-2xl font-bold text-center text-[#006A71] mb-6">Create Your Account</h2>
        ...
        <p className="text-center mt-6 text-sm text-[#006A71]">
          ...
          <Link to="/login" className="font-medium text-[#48A6A7] hover:text-[#006A71] hover:underline transition-colors duration-200">
            Login here
          </Link>
        </p>
      </div>
...
const InputField = ({ icon, name, label, register, required = true, errors, type = 'text', validation }) => (
